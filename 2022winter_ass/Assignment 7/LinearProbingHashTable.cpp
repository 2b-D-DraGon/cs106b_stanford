...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    if(logicSize==allocatedSize) return false;//如果满了则
    else{

//        int index=hashFn(elem)%allocatedSize;
        int index=hashFn(elem);

        if(index>=0&&index<allocatedSize){//如果是在范围内再处理
            ...
            if(elems[index].type==SlotType::EMPTY||elems[index].type==SlotType::TOMBSTONE){
            ...
            else{//如果所要放置的地方已经被霸占了则

                int oldlogicSize=logicSize;

                for(int i=index+1;i<allocatedSize;i++)
                {
                    if(elems[i].type==SlotType::FILLED){//插入到第一个可以放置的地方
                        continue;
                    }
                    elems[i].value=elem;
                ...
                if(oldlogicSize==logicSize){//如果到末尾没有放置则从开头开始寻找
...
//    int index=hashFn(elem)%allocatedSize;
    ...
    if(logicSize==0) return false;

    for(int i=index;i<allocatedSize;i++){
...
//    int index=hashFn(elem)%allocatedSize;
    ...
    if(!contains(elem)||logicSize==0) return false;
...
//            elems[i].value.clear();
    ...
    if(oldlogicSize==logicSize){//如果到末尾没有放置则从开头开始寻找
        for(int i=0;i<index;i++)
        {
            if(elems[i].type==SlotType::FILLED&&elems[i].value==elem){//插入到第一个可以放置的地方
//                elems[i].value.clear();
                elems[i].type=SlotType::TOMBSTONE;
                logicSize--;
                return true;
