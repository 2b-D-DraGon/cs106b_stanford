...
private:
    ...
    void deleteMove(int index){
        ...
        while(1){
            if(elems[(index+1)%allocatedSize].distance==EMPTY_SLOT||elems[(index+1)%allocatedSize].distance==0){
                elems[index].distance=EMPTY_SLOT;
                elems[index].value.clear();
                break;
            }

            elems[index].distance=elems[(index+1)%allocatedSize].distance-1;
            elems[index].value=elems[(index+1)%allocatedSize].value;
