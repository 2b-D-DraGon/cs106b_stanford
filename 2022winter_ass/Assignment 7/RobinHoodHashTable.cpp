...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    for(int i=index+1;i<allocatedSize;i++){
        ...
        if(elems[i].distance==EMPTY_SLOT){//如果为空 则直接插入
            elems[i].value=temp;
            elems[i].distance=i-index;
            logicSize++;
            return true;
        }
        if(elems[i].distance<i-index){//不为空但是距离小于时，将值交换然后将交换出来的值进行判断
            swap(temp,elems[i].value);
            elems[i].distance=i-index;
            insert(temp);
            return true;
        }
    ...
    for(int i=0;i<index;i++){
        ...
        if(elems[i].distance==EMPTY_SLOT){//如果为空 则直接插入
            elems[i].value=temp;
            elems[i].distance=allocatedSize-index+i;
            logicSize++;
            return true;
        }
        if(elems[i].distance<allocatedSize-index+i){//不为空但是距离小于时，将值交换然后将交换出来的值进行判断
            swap(temp,elems[i].value);
            elems[i].distance=allocatedSize-index+i;
            insert(temp);
            return true;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for(int i=0;i<index;i++){
        ...
        if(elems[i].distance<allocatedSize-index+i){
            return false;
        }
        if(elems[i].value==elem){
            return true;
