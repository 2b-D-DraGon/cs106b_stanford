...
HeapPQueue::HeapPQueue() {
    ...
    elems=new DataPoint[allocatedSize+1];//第0个我们设置为空
...
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    elems[logicalSize]=data;//将其插入数组中,数组0的位置不予考虑插入

    bubble_up(logicalSize);//进行将这个数组进行排序

    if(logicalSize==allocatedSize){//已经满了，扩容
        allocatedSize*=2;

        DataPoint* newElems= new DataPoint[allocatedSize+1];//重新开辟一个空间

        for(int i=1;i<logicalSize+1;i++){//将原数组的值全部赋值给新数组
            newElems[i]=elems[i];
        }
...
void HeapPQueue::bubble_up(int index){

    while(index!=1&&elems[index].weight<elems[index/2].weight){

        DataPoint temp=elems[index/2];//与父节点进行交换
...
DataPoint HeapPQueue::peek() const {
    ...
    if(logicalSize==0) error("empty heap");
...
DataPoint HeapPQueue::dequeue() {
    ...
    DataPoint temp=elems[1];

    elems[1]=elems[logicalSize];//将最后一个放入队首的位置

    elems[logicalSize]={};

    logicalSize--;

    bubble_down(1);//进行重新排序


    return temp;
}

void HeapPQueue::bubble_down(int index){



    //如果有两个孩子，并且都大于则跟右边的孩子交换
    if(index*2>logicalSize) return;//没有孩子
    else{//有孩子
        if(2*index+1<=logicalSize){//有右孩子
            if(elems[2*index].weight>elems[2*index+1].weight){//左孩子大于右孩子

                int indexOfLower=2*index+1;
                DataPoint tempOfBub={};

                if(elems[index].weight>elems[indexOfLower].weight){//父节点大于这个较小
                    DataPoint tempOfBub=elems[indexOfLower];//与父节点进行交换
                    elems[indexOfLower]=elems[index];
                    elems[index]=tempOfBub;
                    index=2*index+1;
                    bubble_down(index);//递归处理
                }

            }
            else{//右孩子大于左孩子

                int indexOfLower=2*index;
                DataPoint tempOfBub={};
                ...
                if(elems[index].weight>elems[indexOfLower].weight){//父节点大于这个较小
                    ...
                    elems[index]=tempOfBub;
                    index=2*index;
                    HeapPQueue::bubble_down(index);//递归处理
                }
            ...
            if(elems[index].weight>elems[indexOfLower].weight){//父节点大于左孩子
                DataPoint temp=elems[indexOfLower];//与父节点进行交换
                elems[indexOfLower]=elems[index];
                elems[index]=temp;
                index=2*index;
                bubble_down(index);//递归处理
...
string HeapPQueue::ApportionmentName(){

    double max=elems[1].weight;

    for(int i=2;i<=logicalSize;i++){//找到最大值的位置，最大值一定没有子叶
        if(max<elems[i].weight){
            max=elems[i].weight;
            indexOfApportionmentName=i;
        }
    }

    string name=elems[indexOfApportionmentName].name;

    return name;
}

void HeapPQueue::sort(double num){

        int memory=indexOfApportionmentName;

        elems[memory].weight=num;//更新其节点大小
        //因为一定会变小 所以只用考虑如果大于父节点且与父节点交换后再bubble_down的问题

        while(elems[memory].weight<elems[memory/2].weight&&memory>1){//一直与父节点比较，小于就往上，直到最顶层或者不小于了
            DataPoint temp={};
            temp=elems[memory/2];
            elems[memory/2]=elems[memory];
            elems[memory]=temp;
        }
