...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    ...
    Map<string, int> populationsRec=populations;
    ...
    for(string name: populations){//初始化

        double n=populations[name];

        ret.put(name,1);//一开始所有的设为一

        numSeats--;

        n=n/cal(1);//堆中保存的是一开始所有设为1后的值
    ...
    while(numSeats>0){
        ...
        name=pq.ApportionmentName();//获得最高权重的名字

        ret[name]++;

        numSeats--;

        double n=populations[name];//重新计算权重
        n=n/cal(ret[name]);//堆中保存的是一开始所有设为1后的值
