...
Nucleotide* toStrand(const string& str) {
    ...
    while(i<size){
        if(head==nullptr){
            ...
            cur->next=new Nucleotide();
            cur->next->value=str[i];
            cur->next->prev=cur;
...
bool isMatch(Nucleotide* dna, Nucleotide* target){

    while(dna->value==target->value){

        if(dna->next==nullptr&&target->next!=nullptr){//如果dna到尽头了但是还没匹配完那么不成立
            return false;
        }

        dna=dna->next;
        if(target->next==nullptr){//如果target已经到了尽头那么
            return true;
        }
        target=target->next;
    }
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    if(findFirst(dna,target)==nullptr){
        return false;
    }

    Nucleotide* first=findFirst(dna,target);

    if(dna==first){//如果从开头开始
        if(target==nullptr){
            return true;
        }

        while(target!=nullptr){
            Nucleotide* cur=first;
