...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength<=0) error("please enter a positive num ");

    int n=str.size();

    if(n<kGramLength) return{};
    ...
    for(int i=0;i<n-kGramLength+1;i++){
        temp=word.substr(i,kGramLength);//截取长度kGramLength

        if(kGram.containsKey(temp)){
            kGram[temp]++;
        }
        else{
            kGram.put(temp,1);
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    if(input.isEmpty()) error("please fill the map! ");
    ...
    for(string word:input){
        if(input[word]==0) num++;
        n+=input[word]*input[word];
    }

    if(num==input.size()) error("all nums are zero ! ");
    ...
    for(string word:input){
        mapReturn.put(word,input[word]/n);
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    //遇到一个就塞入，若果超过keep值 则去掉最小的，知道循环完毕，再将优先队列转换成MAP然后返回

    if(numToKeep<0) error("please enter a positive num");
    ...
    PriorityQueue<string> queueOfNmax;

    for(string name: source){
        queueOfNmax.enqueue(name,source[name]);
    }

    while(queueOfNmax.size()>numToKeep){
        queueOfNmax.dequeue();
    }

    Map<string, double> lastOfMap;

    int n = queueOfNmax.size();

    for(int i=0;i<n;i++){
        if(!queueOfNmax.isEmpty()){
            string name = queueOfNmax.dequeue();
            lastOfMap.put(name,source[name]);
        }
    }
...
//        string lowName=toLowerCase(name);

        if(rhs.containsKey(name)){
            valueReturn+=lhs[name]*rhs[name];
        }
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    Map<string, double> returnOfText;
//    Corpus returnOfCorpora;
    returnOfText=normalize(textProfile);//被检测文本的归一化

    for(Corpus returnOfCorpora:corpora){//遍历每一个文本的相似性
        Map<string, double> objectNomalizeMap = normalize(returnOfCorpora.profile);

        double n = cosineSimilarityOf(returnOfText,objectNomalizeMap);//得出余弦相似
        if(n>diff){
            diff=n;
            mostSimilar=returnOfCorpora.name;
        }
    }
