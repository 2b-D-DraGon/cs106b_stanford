...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    for(double heightOfMou:terrain){
        if(heightOfMou>height){
            ifAll = true;//有一个高于水高度就退出
            break;
        }
    }

    if(!ifAll){//全部都低于水的高度,则直接返回都true
        int row = terrain.numRows();
        int col=terrain.numCols();

        Grid<bool> flood(row,col,true);
        return flood;
    }

    Queue<GridLocation> queOfFlood;//记录哪里被一开始淹了
    int row = terrain.numRows();
    int col=terrain.numCols();
    Grid<bool> flood(row,col,false);//创建空的返回网格

    for(GridLocation index : sources){//初始化队列
        if(terrain[index.row][index.col]<height){
            queOfFlood.enqueue(index);//初始条件低于水位排入队列
            flood[index.row][index.col]=true;//设置返回网格的值
        }
    }

    while(!queOfFlood.isEmpty()){
        GridLocation local =queOfFlood.dequeue();
        GridLocation leftLocal ={local.row,local.col-1};
        GridLocation rightLocal ={local.row,local.col+1};
        GridLocation upLocal ={local.row-1,local.col};
        GridLocation downLocal ={local.row+1,local.col};


        if(terrain.inBounds(leftLocal)){
            if(terrain[leftLocal.row][leftLocal.col]<=height&&!flood[leftLocal.row][leftLocal.col]){//当低于水位时并且没有记录时
                queOfFlood.enqueue(leftLocal);//排入队列
                flood[leftLocal.row][leftLocal.col]=true;//设定返回值
            }
        }
        if(terrain.inBounds(rightLocal)){
            if(terrain[rightLocal.row][rightLocal.col]<=height&&!flood[rightLocal.row][rightLocal.col]){//当低于水位时
                queOfFlood.enqueue(rightLocal);//排入队列
                flood[rightLocal.row][rightLocal.col]=true;//设定返回值
            }
        }
        if(terrain.inBounds(upLocal)){
            if(terrain[upLocal.row][upLocal.col]<=height&&!flood[upLocal.row][upLocal.col]){//当低于水位时
                queOfFlood.enqueue(upLocal);//排入队列
                flood[upLocal.row][upLocal.col]=true;//设定返回值
            }
        }
        if(terrain.inBounds(downLocal)){
            if(terrain[downLocal.row][downLocal.col]<=height&&!flood[downLocal.row][downLocal.col]){//当低于水位时
                queOfFlood.enqueue(downLocal);//排入队列
                flood[downLocal.row][downLocal.col]=true;//设定返回值
            }
        }
