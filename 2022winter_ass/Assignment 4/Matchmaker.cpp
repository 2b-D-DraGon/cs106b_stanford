...
bool deleteOnlyPerson(Map<string, Set<string>> &possibleLinks,Set<Pair>& matching,Set<string> &isPairs){

    if(possibleLinks.size()%2!=0){
        return false;
    }

    Vector<string> v =possibleLinks.keys();

    int num=v.size();

    for(int i=0;i<num;i++){
        if(isPairs.contains(v[i])){
            continue;
        }
        if(possibleLinks[v[i]].size()==1){

            string person=v[i];

            if(isPairs.contains(possibleLinks[person].first())){
                return true;
            }

            Pair onlyPair(possibleLinks[person].first(),person);
            matching.add(onlyPair);

            isPairs.add(possibleLinks[person].first());
            isPairs.add(person);

            possibleLinks.remove(possibleLinks[person].first());
...
bool hasPerfectMatchingRec( Map<string, Vector<string>>& v1, Set<Pair>& matching,Set<string> &isPairs,Set<Pair> &matchingTem) {

    if(v1.isEmpty()){
        matching+=matchingTem;
        return true;
    }
    else if(v1.size()==1){
        return false;
    }
    else{//每有一个配对了 就将这个人和配对的消消乐
         //每次对第一个进行判断
        bool finalResult=false;
        string name=v1.firstKey();
        int size=v1[name].size();

        for(int index=0;index<size;index++){
            string conPerson=v1[name][index];
//            Set<string> old1=possibleLinks[name];
            if(isPairs.contains(conPerson)){//如果这个人已经在里面则跳过这次循环，进行下一个人的判断
//                old3=possibleLinks[name];
//                possibleLinks[name].remove(conPerson);
                continue;
            }
//            Set<string> old2=possibleLinks[conPerson];

            Vector<string> vOfKey=v1[name];
            Vector<string> vOfValue=v1[conPerson];

            Pair onlyPair(name,conPerson);//暂且将两个人配对并且加入配对中
            matchingTem.add(onlyPair);

            isPairs.add(name);//将两个人加入已经配对的列表
            isPairs.add(conPerson);

            v1.remove(name);//在剩余队列中删除两人
            v1.remove(conPerson);

            //进行递归
            finalResult=finalResult||hasPerfectMatchingRec(v1,matching,isPairs,matchingTem);

            //恢复
            matchingTem.remove(onlyPair);
            isPairs.remove(name);//将两个人加入已经配对的列表
            isPairs.remove(conPerson);

            v1.put(name,vOfKey);
            v1.put(conPerson,vOfValue);
        }
...
bool isAnyoneSolo(const Map<string, Set<string>>& possibleLinks){
    for(string person:possibleLinks){
        if(possibleLinks[person].isEmpty()) return true;
    }
    return false;
}

void setToVec(Map<string, Set<string>> &set1,Map<string, Vector<string>>& v1){
    Vector<string> v =set1.keys();
    int n=v.size();
    Vector<string> temp;

    for(int index=0;index<n;index++){
        int size= set1[v[index]].size();
        for(int i=0;i<size;i++){
            temp.add(set1[v[index]].first());//取出一个
            set1[v[index]].remove(set1[v[index]].first());//删除一个
        }
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    if(isAnyoneSolo(possibleLinks)){
        return false;
    }

    if(deleteOnlyPerson(Links,matching,isPairs)){
        //只能配对固定一个人的给配对了 只剩下了有很多配对的
    ...
    Set<Pair> matchingTem;

    return hasPerfectMatchingRec(v1,matching,isPairs,matchingTem);
}

void maximumWeightMatchingRec(Map<string, Map<string, int>>& possibleLinks,int tempValue,int& maxValue,Set<Pair> &isPaired,
                              Set<Pair> &maxPaired,Set<Pair> &tempPaired,Set<string> &remainingKeys,Set<string> &isInSet){

    //设计一个容器，当所有的点都被判断过则为0时即开始判断是否是最大的value组
    if(remainingKeys.size()==0){
        if(tempValue>maxValue)
        {
            maxValue=tempValue;

            maxPaired=tempPaired;
            return;
        }
        return;
    }
    else if(possibleLinks.size()==0){
        if(tempValue>maxValue)
        {
            maxValue=tempValue;
    ...
    string name=possibleLinks.firstKey();//取第一个KEY
    Map<string, int> valueOfFirKey=possibleLinks[name];

    Vector<string> firstKeyOfValue=possibleLinks[name].keys();//将name对应的所有的KEY取出来

    int n =possibleLinks[name].size();

    for(int index=0;index<n;index++){//这个情况是将每个都匹配一下试试

        Pair pairs(name,firstKeyOfValue[index]);//每个都配对一下

        if(isPaired.contains(pairs)) continue;//如果配对过就不在配对，进行下一个

        if(isInSet.contains(name)) continue;

        if(isInSet.contains(firstKeyOfValue[index])) continue;


        Map<string, int> valueOfSecKey=possibleLinks[firstKeyOfValue[index]];

        isPaired.add(pairs);//没配对过，将这个配对放进配对过的列表中，防止以后还会发生
        tempPaired.add(pairs);
        //进行下一次的计算

        int num=possibleLinks[name][firstKeyOfValue[index]];

        possibleLinks.remove(name);//删除选中的配对
        possibleLinks.remove(firstKeyOfValue[index]);

        remainingKeys.remove(name);
        remainingKeys.remove(firstKeyOfValue[index]);

        isInSet.add(name);
        isInSet.add(firstKeyOfValue[index]);

        maximumWeightMatchingRec(possibleLinks,tempValue+num,maxValue,isPaired,maxPaired,tempPaired,remainingKeys,isInSet);

        possibleLinks.put(name,valueOfFirKey);//恢复
        possibleLinks.put(firstKeyOfValue[index],valueOfSecKey);

        remainingKeys.add(name);
        remainingKeys.add(firstKeyOfValue[index]);

        isInSet.remove(name);
        isInSet.remove(firstKeyOfValue[index]);

        tempPaired.remove(pairs);
        isPaired.remove(pairs);

    }

//    存在直接跳过不选这个的情况这时只删除这个存在即可
    possibleLinks.remove(name);//删除选中的配对
    remainingKeys.remove(name);
    isInSet.add(name);
    maximumWeightMatchingRec(possibleLinks,tempValue,maxValue,isPaired,maxPaired,tempPaired,remainingKeys,isInSet);
    isInSet.remove(name);
    remainingKeys.add(name);
    possibleLinks.put(name,valueOfFirKey);//恢复
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    Map<string, Map<string, int>> possibleLinksRec=possibleLinks;

    int maxValue=0;

    Set<Pair> isPaired={};//记录是否有过组队

    Set<Pair> maxPaired={};//返回值 组队的最终结果

    Set<Pair> tempPaired={};//每个递归最终递归的结果

    Set<string> isInSet={};

    Vector<string> v1=possibleLinks.keys();

    Set<string> remainingKeys;

    for(int i=0;i<v1.size();i++){
        remainingKeys.add(v1[i]);
    }

    maximumWeightMatchingRec(possibleLinksRec,0,maxValue,isPaired,maxPaired,tempPaired,remainingKeys,isInSet);
