...
EncodingTreeNode* huffmanTreeForRec(PriorityQueue<EncodingTreeNode*> pQUEUE){
    ...
    PriorityQueue<EncodingTreeNode*> newQUEUE;

    if(pQUEUE.size()==1) return pQUEUE.dequeue();

    while(newQUEUE.size()!=2){
        newQUEUE.enqueue(pQUEUE.peek(),pQUEUE.peekPriority());
        pQUEUE.dequeue();
    }

    EncodingTreeNode* newNode=new EncodingTreeNode();

    int priZero=newQUEUE.peekPriority();
    newNode->zero=newQUEUE.dequeue();
    int priOne=newQUEUE.peekPriority();
    newNode->one=newQUEUE.dequeue();

    pQUEUE.enqueue(newNode,priZero+priOne);

    return huffmanTreeForRec(pQUEUE);
}
...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    for(char i:fre){//插入优先队列中
        EncodingTreeNode* newNode=new EncodingTreeNode();
        newNode->ch=i;
        newNode->zero=nullptr;
        newNode->one=nullptr;
        pQUEUE.enqueue(newNode,fre[i]);
    }
...
string decodeTextRec(Queue<Bit>& bits, EncodingTreeNode* tree,string & ret) {
    EncodingTreeNode* root=tree;

    while(1){

        if(bits.isEmpty()) return ret;

        Bit val=bits.dequeue();

        if(val==0){
            tree=tree->zero;
            if(tree->zero==nullptr&&tree->one==nullptr){
                ret+=tree->ch;
                decodeTextRec(bits,root,ret);
            }
        }
        else{
            tree=tree->one;
...
//    return decodeTextRec(bits,tree,ret);
    decodeTextRec(bits,tree,ret);
...
void encodeTextRec(Map<char,Queue<Bit>> &mapTable, EncodingTreeNode* tree,Queue<Bit> path){

    if(tree->zero==nullptr&&tree->one==nullptr){

        mapTable.put(tree->ch,path);
        return;
    }
    if(tree->zero!=nullptr){//左边存在
        Queue<Bit> pathLeft=path;
        pathLeft.enqueue(0);
        encodeTextRec(mapTable,tree->zero,pathLeft);
    }
    if(tree->one!=nullptr){//左边存在
        Queue<Bit> pathRight=path;
        pathRight.enqueue(1);
        encodeTextRec(mapTable,tree->one,pathRight);
    }
...
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    /* TODO: Delete this comment and the next few lines, then implement this. */
    Map<char,Queue<Bit>> mapTable;

    Queue<Bit> path;

    Queue<Bit> ret;

    encodeTextRec(mapTable,tree,path);//获得所有字符的编码映射表

    for(char c:str){

        path=mapTable[c];

        while(!mapTable[c].isEmpty()){
            ret.enqueue(mapTable[c].dequeue());
        }

        while(!path.isEmpty()){
            mapTable[c].enqueue(path.dequeue());
        }
    }
...
void decodeTreeRec(Queue<Bit>& bits, Queue<char>& leaves, EncodingTreeNode* root) {
    /* TODO: Delete this comment and the next few lines, then implement this. */

    Bit val = bits.dequeue();

    if(val==1){//如果有子树
        EncodingTreeNode* NodeLeft=new EncodingTreeNode();
        EncodingTreeNode* NodeRight=new EncodingTreeNode();

        NodeLeft->zero=NodeLeft->one=NodeRight->zero=NodeRight->one=nullptr;

        root->zero=NodeLeft;
        root->one=NodeRight;

        decodeTreeRec(bits,leaves,NodeLeft);
        decodeTreeRec(bits,leaves,NodeRight);
    }
    else{
...
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    decodeTreeRec(bits,leaves,root);
...
HuffmanResult compress(const string& text) {
    ...
    HuffmanResult compre;

    compre.messageBits=encodeText(text,root);

    encodeTree(root,compre.treeBits,compre.treeLeaves);
...
string decompress(HuffmanResult& file) {
    ...
    string ret= decodeText(file.messageBits,root);
