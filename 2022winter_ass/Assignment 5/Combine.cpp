...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<DataPoint> vLeft=combine(left);
    Vector<DataPoint> vRight=combine(right);
    ...
    while(pL<vLeft.size()&&pR<vRight.size()){
        if(vLeft[pL].weight<=vRight[pR].weight){
            v.add(vLeft[pL]);
            pL++;
        }
        else{
