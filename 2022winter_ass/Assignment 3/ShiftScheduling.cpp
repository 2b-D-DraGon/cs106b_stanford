...
void test(){
    Shift a={ Day::WEDNESDAY, 14, 20, 25 };
    Shift b={ Day::WEDNESDAY, 12, 16, 11 };

    bool ret=overlapsWith(a,b);

    int i=0;
}

bool isOverLap(Set<Shift> midSet,Shift B){

    int n=midSet.size();

    for(int i=0;i<n;i++){
        if(overlapsWith(midSet.first(),B)){
            return true;
        }
        midSet.remove(midSet.first());
    }
...
void highestValueScheduleForRec(Set<Shift>& shiftsRemain, int maxHours,
                                Set<Shift> &midSet,Set<Shift> &returnSet,
    ...
    if(maxHours<0){
        error(" maxHour is negetive!");
    }

    if(shiftsRemain.isEmpty()){
        if(Value>maxValue){
            maxValue=Value;
            returnSet=midSet;
        }
        ...
        Shift arrange=shiftsRemain.first();//每次取第一个去决定要或者不要

        if(isOverLap(midSet,arrange)){//如果时间重复，则排除这个时间表
            shiftsRemain.remove(arrange);
            highestValueScheduleForRec(shiftsRemain,maxHours,midSet,returnSet,Value,maxValue);
            shiftsRemain.add(arrange);
            return;
        }
        else if(lengthOf(arrange)<=maxHours){
            //如果空闲时间允许，两种选择，添加进去或者不添加进去
            shiftsRemain.remove(arrange);
            ...
            //添加进去
            midSet.add(arrange);
            highestValueScheduleForRec(shiftsRemain,maxHours-lengthOf(arrange),midSet,returnSet,Value+valueOf(arrange),maxValue);

            //恢复原样
            midSet.remove(arrange);
            shiftsRemain.add(arrange);
        }
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> returnSet={};
    Set<Shift> midSet={};
    Set<Shift> shiftRemaining=shifts;
    int maxValue=0;
    highestValueScheduleForRec(shiftRemaining,maxHours,midSet,returnSet,0,maxValue);
    return returnSet;
}
