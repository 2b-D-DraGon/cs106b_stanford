...
void allEmphasesOfRec(Vector<string> &stringCut,Set<string>& finalRet,string result,int index) {

    if(stringCut.size()==index){
        finalRet.add(result);
        return;
    }

    if(isalpha(stringCut[index][0])){//是字母则进行大写或者不大写的选择
        //进行大写
        allEmphasesOfRec(stringCut,finalRet,result+toUpperCase(stringCut[index]),index+1);

        //进行小写
        allEmphasesOfRec(stringCut,finalRet,result+toLowerCase(stringCut[index]),index+1);//已经裁剪过了第一个
    }
    else{//非字母，则直接添加进去结果
        allEmphasesOfRec(stringCut,finalRet,result+stringCut[index],index+1);
    }
...
Set<string> allEmphasesOf(const string& sentence) {
    ...
    Vector<string> stringCut=tokenize(sentence);
    Set<string> finalRet;
    allEmphasesOfRec(stringCut,finalRet,"",0);
