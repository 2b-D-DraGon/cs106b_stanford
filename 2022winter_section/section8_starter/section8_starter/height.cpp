...
int heightOfRec(Node* root,int height){

    if(root==nullptr){
        return height-1;
    }

    int heightLeft=heightOfRec(root->left,height+1);
    int heightRight=heightOfRec(root->right,height+1);
...
int heightOf(Node* root) {
    ...
    return heightOfRec(root,height);
}
