...
bool isBSTRec(Node* root,int num){
    ...
    if(root->left!=nullptr)
    {
        ...
        if(root->left->data>num) return false;

        if(!isBSTRec(root->left,num)) return false;
    }
    ...
    if(root->right!=nullptr)
    {
        ...
        if(root->right->data<num) return false;

        if(!isBSTRec(root->right,num)) return false;
    }
...
bool isBST(Node* root) {
    ...
    return isBSTRec(root,root->data);
